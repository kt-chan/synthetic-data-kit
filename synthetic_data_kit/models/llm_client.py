...
class LLMClient:
    ...
    def chat_completion(
        ...
        while retry_count < max_retries:
            if self.provider == "api-endpoint":
                llm_output = self._openai_chat_completion(
                    messages, temperature, max_tokens, top_p, verbose
                )
            else:  # Default to vLLM
                llm_output = self._vllm_chat_completion(
                    messages, temperature, max_tokens, top_p, verbose
                )
            ...
            if llm_output and len(llm_output) > 0:  # Example condition to check if output is proper
                break  # Exit the loop if output is proper

            retry_count += 1
            logger.info(f"Retry {retry_count} out of {max_retries}. Trying again...")

        if len(llm_output) == 0 and retry_count == max_retries:
    ...
    def batch_completion(
        ...
        while retry_count < max_retries:
            ...
            # Check if the output is proper. You need to define what "proper" means for your case.
            if llm_output and len(llm_output) > 0:  # Example condition to check if output is proper
                logger.info(f"batch_completion generated { len(llm_output) } outputs")
                break  # Exit the loop if output is proper
    ...
    def _openai_batch_completion(
        ...
        for i in range(0, len(message_batches), batch_size):
            ...
            if verbose:
                logger.info(
                    f"Processing inference batch {i//batch_size + 1}/{(len(message_batches) + batch_size - 1) // batch_size} with {len(batch_chunk)} requests"
                )
